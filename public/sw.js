...
// ── Coin Sound (khas Cashhero — koin jatuh ke celengan) ─────────────────────
function playCoinSound() {
  try {
    ...
    coinTimes.forEach((delay, idx) => {
      ...
      iSrc.connect(bp); bp.connect(iGain); iGain.connect(master);
      iSrc.start(t); iSrc.stop(t + iDur);
      ...
      thOsc.connect(thGain); thGain.connect(master);
      thOsc.start(t + 0.008); thOsc.stop(t + 0.05);
      ...
      sOsc.connect(sGain); sGain.connect(master);
      sOsc.start(t + 0.003); sOsc.stop(t + 0.015);
      ...
      if (idx < coinTimes.length - 1 && Math.random() > 0.3) {
        ...
        bSrc.connect(bGain); bGain.connect(master);
        bSrc.start(bt); bSrc.stop(bt + bDur);
      }
    });
    ...
    cOsc.connect(cGain); cGain.connect(master);
    cOsc.start(now); cOsc.stop(now + 0.45);
    ...
    for (let s = 0; s < 20; s++) {
      ...
      sOsc.connect(sGain); sGain.connect(master);
      sOsc.start(st); sOsc.stop(st + 0.012);
    }
    ...
    fOsc.connect(fGain); fGain.connect(master);
    fOsc.start(now + 0.30); fOsc.stop(now + 0.50);
  } catch (e) {
    console.log('[sw.js] Coin sound error:', e);
  }
}
...
async function tryShowAuditReport() {
  ...
  const lang = data.language || storedLang
...
self.addEventListener('message', (event) => {
  ...
  if (data.type === 'SET_LANG') {
    storedLang = data.payload?.language || 'en'
    return
  }
  ...
  if (action && action.startsWith('recurring-')) {
    ...
    event.waitUntil(
      self.clients.matchAll({ type: 'window' }).then((clients) => {
        ...
        // No open client: open app with action data in hash
