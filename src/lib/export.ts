...
function fmtCurrency(num: number, language: 'id' | 'en', currency: string, rate: number): string {
...
function formatAmount(num: number, language: 'id' | 'en'): string {
  const { currency, rate } = getCurrencyInfo()
  return fmtCurrency(num, language, currency, rate)
}
...
export function exportToExcel(
  ...
  const title = isId ? "LAPORAN KEUANGAN CASHHERO" : "CASHHERO FINANCIAL REPORT"
  ...
  const { currency, rate } = getCurrencyInfo()
  const fmt = (num: number) => fmtCurrency(num, language, currency, rate)

  const rows = transactions.map((t, idx) => {
    const formattedDate = new Date(t.date).toLocaleDateString(isId ? 'id-ID' : 'en-US', {
      year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit'
    })
    const typeLabel = t.type === 'in' ? (isId ? "Pemasukan" : "Income") : (isId ? "Pengeluaran" : "Expense")
  ...
  const html = `
    ...
    <head>
      ...
      <![endif]-->
      <style>
        @page { margin: 0.75in 0.75in 0.75in 0.75in; mso-header-margin: 0.3in; mso-footer-margin: 0.3in; }
      </style>
    </head>
    <body style="font-family: 'Segoe UI', Arial, sans-serif; margin: 0; padding: 20px; background-color: #FFFFFF;">
      <table style="border-collapse: collapse; width: 100%;">
        <colgroup>
          <col width="180" /><col width="160" /><col width="300" /><col width="130" /><col width="180" />
        </colgroup>
        <tr>
        ...
        <tr style="background-color: #1E293B;">
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: center; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[0]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: left; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[1]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: left; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[2]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: center; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[3]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: right; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[4]}</th>
        </tr>
        ${rows}
  ...
  downloadExcelBlob(html, `Cashhero_Report_${sanitizedFilter}_${dateStr}.xls`)
}
...
export function exportToPDF(
  ...
  const fmt = (num: number) => formatAmount(num, language)
    ...
    <html>
      <head>
        ...
        <style>
          ...
          body { font-family: 'Inter', Arial, sans-serif; color: #1E293B; margin: 0; padding: 0; background: #FFFFFF; -webkit-print-color-adjust: exact; print-color-adjust: exact; font-size: 11px; line-height: 1.4; }
          .header-container { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 2px solid #0F172A; padding-bottom: 12px; margin-bottom: 20px; }
          .header-left .company-name { font-size: 22px; font-weight: 800; color: #810B38; letter-spacing: -0.5px; text-transform: uppercase; margin: 0 0 2px 0; }
          .header-left .document-type { font-size: 12px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.5px; margin: 0; }
          .header-right { text-align: right; font-size: 10px; color: #64748B; }
          .header-right .verified-badge { display: inline-block; padding: 4px 8px; background: #F0FDF4; border: 1px solid #BBF7D0; color: #16A34A; border-radius: 6px; font-weight: 700; font-size: 9px; text-transform: uppercase; margin-bottom: 6px; }
          .meta-grid { display: grid; grid-template-columns: 2fr 1fr; gap: 20px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 10px 14px; margin-bottom: 24px; font-size: 11px; }
          ...
          .metric-card { padding: 12px 14px; border-radius: 8px; border: 1px solid #E2E8F0; background: #F8FAFC; }
          .metric-label { font-size: 9px; font-weight: 700; text-transform: uppercase; color: #64748B; letter-spacing: 0.5px; display: block; margin-bottom: 4px; }
          .metric-value { font-size: 15px; font-weight: 800; letter-spacing: -0.3px; }
          table { width: 100%; border-collapse: collapse; margin-top: 15px; page-break-inside: auto; }
          thead { display: table-header-group; }
          tr { page-break-inside: avoid; page-break-after: auto; }
          th { padding: 10px 8px; text-align: left; background: #0F172A; color: #FFFFFF; font-weight: 700; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid #475569; font-family: 'Inter', sans-serif; }
          .text-center { text-align: center; }
          .text-right { text-align: right; }
          .total-row td { border-top: 1.5px solid #0F172A; border-bottom: 3px double #0F172A; font-weight: 800; padding: 12px 8px; font-size: 12px; color: #0F172A; background: #F8FAFC; }
          .footer { position: fixed; bottom: 0; left: 0; right: 0; text-align: center; font-size: 8px; color: #94A3B8; border-top: 1px solid #E2E8F0; padding-top: 6px; page-break-before: avoid; }
        </style>
      </head>
      <body>
...
export function exportAssetHistoryToExcel(
  ...
  const rows = [...logs].reverse().map((log, idx) => {
    ...
    const typeLabel = { capital_change: isId ? "Modal" : "Capital", profit: isId ? "Keuntungan" : "Profit", loss: isId ? "Kerugian" : "Loss", liquidation: isId ? "Likuidasi" : "Liquidation" }[log.type]
    const typeColor = { capital_change: '#1D4ED8', profit: '#15803D', loss: '#B91C1C', liquidation: '#B45309' }[log.type]
    const typeBg = { capital_change: '#EFF6FF', profit: '#DCFCE7', loss: '#FEE2E2', liquidation: '#FEF3C7' }[log.type]
    const rowBg = idx % 2 === 0 ? '#FFFFFF' : '#F8FAFC'
  ...
  const html = `
    ...
    <body style="font-family: 'Segoe UI', Arial, sans-serif; margin: 0; padding: 20px; background-color: #FFFFFF;">
      <table style="border-collapse: collapse; width: 100%;">
        <colgroup>
          <col width="180" /><col width="140" /><col width="300" /><col width="180" />
        </colgroup>
        <tr>
        ...
        <tr style="background-color: #1E293B;">
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: center; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[0]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: center; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[1]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: left; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[2]}</th>
          <th style="border: 1px solid #475569; color: #FFFFFF; font-size: 10pt; font-weight: bold; text-align: right; padding: 12px; font-family: 'Segoe UI', Arial, sans-serif;">${headers[3]}</th>
        </tr>
        ${rows}
        <tr style="background-color: #F1F5F9;">
  ...
  const dateStr = new Date().toISOString().split('T')[0]
  downloadExcelBlob(html, `Riwayat_Aset_${sanitizedAssetName}_${dateStr}.xls`)
}
...
export function exportAssetHistoryToPDF(
  ...
  const rowsHtml = [...logs].reverse().map((log, idx) => {
    ...
    const typeLabel = { capital_change: isId ? "Modal" : "Capital", profit: isId ? "Keuntungan" : "Profit", loss: isId ? "Kerugian" : "Loss", liquidation: isId ? "Likuidasi" : "Liquidation" }[log.type]
    const typeColor = { capital_change: '#1D4ED8', profit: '#15803D', loss: '#B91C1C', liquidation: '#D97706' }[log.type]
    const typeBg = { capital_change: '#EFF6FF', profit: '#F0FDF4', loss: '#FEF2F2', liquidation: '#FEF3C7' }[log.type]
    const typeBorder = { capital_change: '#BFDBFE', profit: '#BBF7D0', loss: '#FCA5A5', liquidation: '#FDE68A' }[log.type]
    const rowBg = idx % 2 === 0 ? '#FFFFFF' : '#F8FAFC'
    ...
    <html>
      <head>
        ...
        <style>
          ...
          .header-left .document-type { font-size: 12px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: 0.5px; margin: 0; }
          .header-right { text-align: right; font-size: 10px; color: #64748B; }
          .header-right .verified-badge { display: inline-block; padding: 4px 8px; background: #EFF6FF; border: 1px solid #BFDBFE; color: #1D4ED8; border-radius: 6px; font-weight: 700; font-size: 9px; text-transform: uppercase; margin-bottom: 6px; }
          .meta-grid { display: grid; grid-template-columns: 2fr 1fr; gap: 20px; background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 10px 14px; margin-bottom: 24px; font-size: 11px; }
          .meta-item { margin-bottom: 4px; }
          .meta-item:last-child { margin-bottom: 0; }
          .meta-label { color: #64748B; font-weight: 600; }
          ...
          .metric-card { padding: 10px 12px; border-radius: 8px; border: 1px solid #E2E8F0; background: #F8FAFC; }
          .metric-label { font-size: 8px; font-weight: 700; text-transform: uppercase; color: #64748B; letter-spacing: 0.5px; display: block; margin-bottom: 4px; }
          .metric-value { font-size: 12px; font-weight: 800; letter-spacing: -0.3px; }
          table { width: 100%; border-collapse: collapse; margin-top: 15px; page-break-inside: auto; }
          thead { display: table-header-group; }
          tr { page-break-inside: avoid; page-break-after: auto; }
          th { padding: 10px 8px; text-align: left; background: #0F172A; color: #FFFFFF; font-weight: 700; font-size: 10px; text-transform: uppercase; letter-spacing: 0.5px; border-bottom: 2px solid #475569; font-family: 'Inter', sans-serif; }
